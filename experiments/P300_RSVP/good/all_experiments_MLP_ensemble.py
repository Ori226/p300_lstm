...
class LSTM_EEG_Ensamble(GeneralModel):
    def get_params(self):
        super(LSTM_EEG_Ensamble, self).get_params()
        return self.model.get_weights()

    def get_name(self):
        super(LSTM_EEG_Ensamble, self).get_name()
        return self.__class__.__name__ + "_" + str(self._num_of_hidden_units) + "_" + str(self.positive_weight)

    def reset(self):
        super(LSTM_EEG_Ensamble, self).reset()
        self.model.set_weights(self.original_weights)

    def __init__(self, positive_weight, _num_of_hidden_units):
        super(LSTM_EEG_Ensamble, self).__init__()
        self.positive_weight = positive_weight
        self._num_of_hidden_units = _num_of_hidden_units
        self.number_of_subset = 29
        '''
        ...
        self.model = Sequential()
        self.model.add(LSTM(input_dim=55, output_dim=_num_of_hidden_units, input_length=number_of_timestamps,
    ...
    def fit(self, _X, y):
        ...
        for X_sub_db, y_sub_db in zip(_X,y):
            ...
            self.model.set_weights(self.all_weight[counter])
            self.model.fit(stats.zscore(X_sub_db, axis=1), _y,
                           nb_epoch=20, batch_size=128, show_accuracy=True, verbose=1)
            self.all_weight[counter] = self.model.get_weights()
            counter += 1
...
class MLP_EEG_Ensamble(GeneralModel):
    def get_params(self):
        super(MLP_EEG_Ensamble, self).get_params()
        return self.model.get_weights()

    def get_name(self):
        super(MLP_EEG_Ensamble, self).get_name()
        return self.__class__.__name__ + "_" + str(self._num_of_hidden_units) + "_" + str(self.positive_weight)

    def reset(self):
        super(MLP_EEG_Ensamble, self).reset()
        self.model.set_weights(self.original_weights)

    def __init__(self, positive_weight, _num_of_hidden_units):
        super(MLP_EEG_Ensamble, self).__init__()
        self.positive_weight = positive_weight
        self._num_of_hidden_units = _num_of_hidden_units
        self.number_of_subset = 29
        '''
...
if __name__ == "__main__":
    ...
    all_models = [MLP_EEG_Ensamble(1,100)]
