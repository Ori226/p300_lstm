from P300Net.data_preparation import create_data_rep_training, triplet_data_generator_no_dict, triplet_data_collection_no_dict
from P300Net.models import get_item_subgraph, get_graph,get_graph_lstm,  get_item_lstm_subgraph, identity_loss
...
if __name__ == "__main__":
    ...
    for subject in all_subjects:
        ...
        valid_data = triplet_data_collection_no_dict(all_data_per_char_as_matrix[train_mode_per_block == 2],
        ...
        # model.add(Dense(1))
        # P300IdentificationModel = model
        model.compile(loss=identity_loss, class_mode="binary", optimizer='rmsprop')
        # endregion
        next_batch = data_generator.next()
        temp = model.evaluate(next_batch[0], next_batch[1], batch_size=len(next_batch[0]))
        # region train the P300Net model
        # model.fit_generator(data_generator, 2880, nb_epoch=10, validation_data=valid_data)
        model.fit_generator(data_generator, 80*5, nb_epoch=20)
        # endregion
        ...
        actual = np.where(np.round(softmax_res_func(np.mean(all_prediction_P300Net.reshape((-1, 10, 30)), axis=1))) == 1)[0];
