...
def train_p300_model(model, train_data_as_matrix, train_tags_as_matrix):
    ...
    shuffle_flatten_data, shuffle_flatten_tags, = flatten_data, flatten_tags

    model.fit(scipy.stats.zscore(shuffle_flatten_data,axis=1), shuffle_flatten_tags, nb_epoch=20,shuffle=False)
...
if __name__ == "__main__":
    ...
    for subject in all_subjects:
        ...
        actual = np.where(np.round(softmax_res_func(np.mean(all_prediction_normal.reshape((-1, 10, 30)), axis=1))) == 1)[0];
