...
if __name__ == "__main__":
    ...
    for experiment_counter, subject in enumerate(all_subjects):
        ...
        (shuffle_train_data, shuffled_tags) = (train_data, train_tags)#shuffle(train_data, train_tags, random_state=42)
        sample_weigt = np.ones_like(shuffled_tags)
        sample_weigt[shuffled_tags] = 1
        # model = get_P300_model(only_p300_model_1, select=1)
        model.set_weights(np.load(r'c:\temp\weights_42.npy'))


        model.fit(shuffle_train_data,shuffled_tags,
                  batch_size=600, nb_epoch=20, callbacks=[history], shuffle=True,sample_weight=sample_weigt)
        # np.save(r'c:\temp\weights_43.npy', model.get_weights())
        temp = history.losses
