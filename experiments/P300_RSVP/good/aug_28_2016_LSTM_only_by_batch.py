...
def identity_loss_v3(y_true, y_pred):
    import theano
    y_true_reshaped = K.mean(K.reshape(y_true, (-1, 2, 30)), axis=1)
    y_pred_reshaped = K.softmax(K.mean(K.reshape(y_pred, (-1, 2, 30)), axis=1))
...
if __name__ == "__main__":
    ...
    for subject in all_subjects:
        ...
        data_generator = triplet_data_generator_no_dict(all_data_per_char_as_matrix[train_mode_per_block == 1], target_per_char_as_matrix[train_mode_per_block == 1], 20,3)
        ...
        model = Sequential()
        # model.add(Flatten(input_shape=(number_of_timestamps, 55)))
        ...
        # model.add(Lambda(lambda x: x**1, batch_input_shape=first_batch.shape))
        # model.add(Lambda(simple_function, batch_input_shape=first_batch.shape))


        # imd_f = K.function([model.input],
        #                    model.layers[0].output)




        # temp = imd_f([np.array(first_batch.reshape(-1), dtype='float32')])
        ...
        # number_of_samples_in_epoch = 270
        model.fit_generator(data_generator,36000, nb_epoch=1)
        #
        #
        # x1 = np.array([[100,0,0]],dtype=np.float32)
        # x2 = np.array([[100, 0, 0]], dtype=np.float32)
        # import theano
        # x = T.dmatrix('x')
        # y = T.dmatrix('y')
        # softmax_res_func = theano.function([x,y], T.nnet.categorical_crossentropy(T.nnet.softmax(x),T.nnet.softmax(y)) )
        # temp = softmax_res_func(x1,x2)

        # model.fit(first_batch, first_batch, nb_epoch=1)
