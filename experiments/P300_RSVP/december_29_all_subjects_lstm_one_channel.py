...
def get_only_P300_model_LSTM_single_channel(eeg_sample_shape):
    from keras.regularizers import l2
    digit_input = Input(shape=eeg_sample_shape)

    # break the input in 55 models
    # x = Flatten(input_shape=eeg_sample_shape)(digit_input)
    x1 = Lambda(lambda x: x[:,:,0:2], output_shape=( 25,2))(digit_input)
    # x2 = Lambda(lambda x: x[:, :, 0:2], output_shape=(25, 2))(digit_input)

    input_sub = Input(shape=(25, 1))
    single_channle_classifier = LSTM(20,return_sequences=True)(input_sub)
    single_channle_classifier = LSTM(20, return_sequences=False)(single_channle_classifier)
    single_channle_classifier = Dense(1, activation='sigmoid')(single_channle_classifier)

    single_channel_model = Model(input_sub, single_channle_classifier)

    # x = noise.GaussianNoise(sigma=0.0)(digit_input)
    # for i in range(55):
    # x1 = single_channel_model(x1)
    # x2 = single_channel_model(x2)

    # all_sub_chann_layers = [Lambda(lambda x: x[:, :, i:i+1], output_shape=(25, 1))(digit_input) for i in range(2)]
    x = single_channel_model(Lambda(lambda x: x[:, :, 0:0 + 1], output_shape=(25, 1))(digit_input))


    # x = merge([single_channel_model(sub_chann) for  sub_chann  in all_sub_chann_layers ],mode='ave')
...
def get_only_P300_model_CNN(eeg_sample_shape):
    ...
    # self.model.add(Reshape((1, number_of_time_stamps, number_of_in_channels),
    #                        input_shape=(number_of_time_stamps, number_of_in_channels)))
...
if __name__ == "__main__":
    ...
    only_p300_model_1 = get_only_P300_model_LSTM_single_channel(eeg_sample_shape)
    ...
    temp = only_p300_model_1.predict(train_data.reshape(train_data.shape[0] * train_data.shape[1], train_data.shape[2], train_data.shape[3]))

    for i in  range(20):
        model.fit(train_data.reshape(train_data.shape[0] * train_data.shape[1], train_data.shape[2], train_data.shape[3]), train_tags,verbose=1,nb_epoch=1, batch_size=600,shuffle=True)
